...
toggle_waybar_border() {
    ...
    if [[ "$enable" == "y" || "$enable" == "Y" ]]; then
        ...
        sudo sed -i 's|background-color: rgba(23,23,23,0);|background-color: rgba(23,23,23,0.5);|' "$waybar_style_file"
        sudo sed -i 's|^\([[:space:]]*\)/\*border: 1px solid @bordercolor;\*/|\1border: 1px solid @bordercolor;|' "$waybar_style_file"
        echo -e "${GREEN}✓ Waybar border/background enabled.${RESET}"
        border_enabled=true
    else
        # Disable background and comment out border
        sudo sed -i 's|background-color: rgba(23,23,23,0.5);|background-color: rgba(23,23,23,0);|' "$waybar_style_file"
        sudo sed -i 's|^\([[:space:]]*\)border: 1px solid @bordercolor;|\1/*border: 1px solid @bordercolor;*/|' "$waybar_style_file"
